module fp_div #(parameter int WIDTH=64)(
    ...
    localparam int QUOT_WIDTH = SIG_WIDTH + 4;
    localparam int NUM_WIDTH = SIG_WIDTH + QUOT_WIDTH;
    ...
    always_comb begin
        ...
        // Normal division
            ...
            quotient_full = numerator/sig_b_norm;
            remainder_full = numerator%sig_b_norm;
            quotient = quotient_full[QUOT_WIDTH-1:0];

            // Normalize quotient

            if (quotient[QUOT_WIDTH-1])
                quotient_norm = quotient;
            else begin
                quotient_norm = quotient << 1;
                exp_result_unbiased = exp_result_unbiased-1;
            end

            // Extract significand and rounding bits

            sig_main = quotient_norm[QUOT_WIDTH-1 -: SIG_WIDTH];
            guard_bit = quotient_norm[3];
            round_bit = quotient_norm[2];
            sticky_bit = (|quotient_norm[1:0]) || (remainder_full != 0);
            ...
            // Normal result
                ...
                rounded_ext = {1'b0,sig_main}+round_up;
